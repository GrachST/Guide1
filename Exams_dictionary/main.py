...
def resource_path(relative_path):
    try:
        base_path = sys._MEIPASS  # Путь при сборке с PyInstaller
    except Exception:
        base_path = os.path.abspath(".")  # Путь при обычном запуске
...
pygame.display.set_caption("Словарь ОГЭ и ЕГЭ")
...
dictionary = load_dictionary(resource_path('dictionary.json'))

# Обработка математических выражений
def convert_to_sympy(data):
    if isinstance(data, dict):
        return {key: convert_to_sympy(value) for key, value in data.items()}
    elif isinstance(data, list):
        return [convert_to_sympy(item) for item in data]
    elif isinstance(data, str):
        try:
            # Попытаться преобразовать строку в математическое выражение
            expr = sp.sympify(data)
            return str(expr)  # Возвращаем строку после преобразования
        except:
            return data  # Если не удалось преобразовать, возвращаем как есть
    else:
        return data
...
def update_suggestions(text):
    ...
    if len(text) >= 2:  # Подсказки появляются при вводе двух и более символов
        ...
        if event.type == pygame.KEYDOWN:
            if active:
                if event.key == pygame.K_RETURN:
                    ...
                    # Добавляем любые символы, включая юникод
                ...
                # Обновление подсказок после изменения текста
